...
class LcarsText():
    ...
    def __init__(self, canvasroot, colour, pos, text, size=1.0, bg_colour = colours.BLACK, anchor=tk.W, width=None):
        ...
        self.textItem.place(x=self.pos[0], y=self.pos[1])
        if self.width is not None:
            self.textItem.config(width=self.width)
    ...
    def isClock(self):
        ...
        # Clock Time Loop
        self.anchor = tk.E
        self.textItem.config(anchor=tk.E)
        self.tick_time1 = time.strftime(self.clockformat)
        self.textItem.config(text=self.tick_time1)
        self.textItem.after(200, self.__clockTimeUpdate)
    ...
    def isDate(self):
        ...
        self.textItem.config(width=28)
        self.textItem.config(anchor=tk.CENTER)
        self.textItem.config(text=time.strftime(self.dateformat))
        self.textItem.after(20000, self.__clockDateUpdate)
    ...
    def __clockTimeUpdate(self):
        ...
        try:
            ...
            if tick_time2 != self.tick_time1:
                ...
                self.textItem.config(text=tick_time2)
